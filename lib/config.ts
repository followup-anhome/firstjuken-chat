/**
 * firstjuken-chat（ファースト住建専用）
 * 機能設定ファイル
 *
 * このリポジトリは firstjuken-chat（ファースト住建専用）。
 * デフォルトモード "first" 固定（NEXT_PUBLIC_MODE 未設定でも first で起動）。
 *
 * 他モード（full/demo/school/uno/followup）の設定は
 * 過去の互換性のため残されているが実行時には参照されない。
 * （5/13以降にクリーンアップ予定 → CLAUDE.md 参照）
 *
 * NEXT_PUBLIC_MODE で動作モードを切り替え（参考）：
 *   first   → ファースト住建（デフォルト・本番）
 *   full    → 全機能
 *   demo    → 企業デモ用
 *   school  → 佐野中学校 夜間学級用
 *   uno     → UNO専用
...
export const MODE: AppMode = raw ?? "first";
